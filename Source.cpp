...
int GetScreeny(HWND hMyWnd, LPWSTR lpszFilename, WCHAR* format, ULONG uQuality) // by Napalm
{
	...
	HDC hdc = GetWindowDC(hMyWnd);
	int width = rect.right - rect.left;
	int height = rect.bottom - rect.top;
	int nBPP = GetDeviceCaps(hdc, BITSPIXEL);
	HDC hdcCapture = CreateCompatibleDC(hdc);
	BITMAPINFO bmiCapture = { sizeof(BITMAPINFOHEADER), width, -height, 1, (WORD)nBPP, BI_RGB, 0, 0, 0, 0, 0 };
	LPBYTE lpCapture;
	HBITMAP hbmCapture = CreateDIBSection(hdc, &bmiCapture, DIB_PAL_COLORS, (LPVOID*)&lpCapture, NULL, 0);
	if (!hbmCapture)
	{
		DeleteDC(hdcCapture);
		DeleteDC(hdc);
	...
	BitBlt(hdcCapture, 0, 0, width, height, hdc, 0, 0, SRCCOPY);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	static HWND hEdit3;
	static WCHAR szINIFilePath[MAX_PATH];
	static WCHAR szFileNameTemplate[MAX_PATH];
	static DWORD dwFileType;
	static WCHAR szSaveDirPath[MAX_PATH];
	static DWORD dwTimeSpan;
	switch (msg)
	{
	case WM_CREATE:
	{
		...
		GetPrivateProfileString(L"AUTO_CAPTURE", L"FILE_NAME_TEMP", L"%Y_%M_%D_%h_%m_%s", szFileNameTemplate, MAX_PATH, szINIFilePath);
		dwFileType = GetPrivateProfileInt(L"AUTO_CAPTURE", L"FILE_TYPE", 0, szINIFilePath);
		GetPrivateProfileString(L"AUTO_CAPTURE", L"SAVE_DIR", L"", szSaveDirPath, MAX_PATH, szINIFilePath);
		dwTimeSpan = GetPrivateProfileInt(L"AUTO_CAPTURE", L"TIME_SPAN", 5, szINIFilePath);
	...
	case WM_COMMAND:
		if (LOWORD(wParam) == IDOK)
		{
			...
			WritePrivateProfileString(L"AUTO_CAPTURE", L"FILE_NAME_TEMP", szFileNameTemplate, szINIFilePath);
			WCHAR szTemp[128];
			wsprintf(szTemp, L"%d", dwFileType);
			WritePrivateProfileString(L"AUTO_CAPTURE", L"FILE_TYPE", szTemp, szINIFilePath);
			WritePrivateProfileString(L"AUTO_CAPTURE", L"SAVE_DIR", szSaveDirPath, szINIFilePath);
			wsprintf(szTemp, L"%d", dwTimeSpan);
			WritePrivateProfileString(L"AUTO_CAPTURE", L"TIME_SPAN", szTemp, szINIFilePath);
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nShowCmd)
{
	...
	MSG msg;
	WNDCLASS wndclass = { CS_HREDRAW | CS_VREDRAW,WndProc,0,DLGWINDOWEXTRA,hInstance,LoadIcon(hInstance, MAKEINTRESOURCE(IDI_ICON1)),LoadCursor(0,IDC_ARROW),(HBRUSH)(COLOR_WINDOW + 1),0,szClassName };
	RegisterClass(&wndclass);
	HWND hWnd = CreateWindow(szClassName, L"auto capture", WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN, CW_USEDEFAULT, 0, CW_USEDEFAULT, 0, 0, 0, hInstance, 0);
